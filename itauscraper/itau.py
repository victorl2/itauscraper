...
def __account_file(file_path: str) -> str:
...
def __credentials_file(file_path: str) -> str:
...
@click.command()
@click.argument('agencia', type=str, required=True)
@click.argument('conta', type=str, required=True)
@click.argument('senha', type=str, required=True)
def login(agencia: str, conta: str, senha: str) -> None:
    print(agencia, conta, senha)
    if not senha.isnumeric() or len(senha) != 6:
